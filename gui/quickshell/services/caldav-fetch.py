...
def main():
    ...
    ap.add_argument("--calendars", default="", metavar="NAMES",
                    help="comma-separated calendar display names to include (default: all)")
    ap.add_argument("--past-days", type=int, default=62,
                    help="how many days before today to fetch (default: 62)")
    ap.add_argument("--future-days", type=int, default=186,
    ...
    try:
        ...
        sys.exit(f"caldav-fetch: connection failed: {e}")
    ...
    for cal in calendars:
        ...
        for obj in ics_objects:
            try:
                ...
                print(f"caldav-fetch: skipping broken event in {name!r}: {e}",
                      file=sys.stderr)
